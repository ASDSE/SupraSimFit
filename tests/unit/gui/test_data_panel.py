...
JASCO_FIXTURE = Path(__file__).parent.parent.parent / 'data' / 'jasco' / 'cb7in30umbc_1-1.csv'
...
def loaded_panel(qapp, tmp_path):
    ...
    df = pd.DataFrame(
        [{'concentration': c, 'signal': (i + 1) * 100.0, 'replica': r} for r in range(2) for i, c in enumerate(conc)]
    )
...
class TestImportedUnitReinterpret:
    ...
    def test_unit_change_does_not_rewrite_table_values(self, loaded_panel):
        before = [loaded_panel._conc_table.item(r, 0).text() for r in range(loaded_panel._conc_table.rowCount())]
        loaded_panel._imported_unit_combo.setCurrentText('µM')
        after = [loaded_panel._conc_table.item(r, 0).text() for r in range(loaded_panel._conc_table.rowCount())]
        assert before == after
...
class TestChannelCombo:
    ...
    def test_switch_preserves_entered_concentrations(self, multi_channel_panel):
        ...
        assert not multi_channel_panel.measurement_set().metadata.get('bmg_placeholder_concentrations')
